...
void RGEngine::init()
{
    ...
    // Bake world transforms once so the physics build can read each node's world pose.
    if (auto rootNode = std::dynamic_pointer_cast<sgraph::Node>(scenegraph->getRoot()))
    {
        rootNode->refreshTransform(glm::mat4{1.f});
    }

    // Build Box3D bodies from the parsed rigidbody specs.
    physics.init();
...
void RGEngine::update_scene()
{
    ...
    // Directional shadow map: an orthographic "camera" at the sun looking at the scene.
    {
    ...
    // Draw the authored scenegraph (leaf world transforms already reflect the physics sync).
    ...
    // Feed collider wireframes to the debug overlay.
    if (debugFeature)
    {
